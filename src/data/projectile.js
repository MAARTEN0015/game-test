...
export default function Projectile(players, ais, objectManager, items, config, UTILS, server) {
            ...
            if (server) {
                for (var i = 0; i < players.length; ++i) {
                    if (!this.sentTo[players[i].id] && players[i].canSee(this)) {
                        ...
                        server.send(players[i].id, PACKETS.SERVER.ADD_PROJECTILE, UTILS.fixTo(this.x, 1), UTILS.fixTo(this.y, 1),
                ...
                if (objectsHit.length > 0) {
                                ...
                                if (hitObj.sentTo[players[i].id]) {
                                    if (hitObj.active) {
                                        if (players[i].canSee(hitObj))
                                            server.send(players[i].id, PACKETS.SERVER.WIGGLE_GAME_OBJECT, UTILS.fixTo(this.dir, 2), hitObj.sid);
                                    } else {
                                        server.send(players[i].id, PACKETS.SERVER.KILL_OBJECT, hitObj.sid);
                                    }
                                }
                    ...
                    for (var i = 0; i < players.length; ++i) {
                        if (this.sentTo[players[i].id])
                            server.send(players[i].id, PACKETS.SERVER.REMOVE_PROJECTILE, this.sid, UTILS.fixTo(shortDist, 1));
                    }
                }
            }
        }
